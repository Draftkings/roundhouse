namespace roundhouse.tests.infrastructure.containers
{
    ...
    public class DefaultDatabaseMigratorSpecs
    {
        ...
        public abstract class concern_for_dangerous_Migrate : observations_for_a_sut_without_a_contract<DefaultDatabaseMigrator>
        {
            protected static object result;
            protected static DefaultEnvironment environment;
            protected static string dangerous_drop_sql = @"
USE `RoundhousE`;

DROP TABLE IF EXISTS SomeTable;

CREATE TABLE SomeTable(
	SomeTableId INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	StatusId INT NOT NULL,
	UserId INT NOT NULL,
	UserAddressId INT NOT NULL,
	CreatedOnUtc TIMESTAMP DEFAULT CURRENT_TIMESTAMP NOT NULL,
	INDEX `IX_SomeTable_UserId` (`UserId`)
);

DROP TABLE IF EXISTS SomeTableItem;

CREATE TABLE SomeTableItem(
	SomeTableItemId INT NOT NULL PRIMARY KEY AUTO_INCREMENT,
	SomeTableId INT NOT NULL,
	ItemId INT NOT NULL,
	INDEX `IX_SomeTableItemId_SomeTableId` (`SomeTableId`)
);
";

            protected static string dangerous_truncate_sql = @"
USE `RoundhousE`;
...
Truncate SomeTableItem;
";
            
            protected static string dangerous_sql_with_override = @"
USE `RoundhousE`;
...
";
        ...
        [Concern(typeof(DefaultDatabaseMigrator))]
        public class when_running_the_migrator_with_dangerous_scripts : concern_for_dangerous_Migrate
        {
            private context c = () => { };
            ...
            [Observation]
            public void if_script_contains_drop_should_return_true()
            {
                sut.is_dangerous_unreviewed_script_found(dangerous_drop_sql).should_be_true();
            }

            [Observation]
            public void if_script_contains_truncate_should_return_true()
            {
                sut.is_dangerous_unreviewed_script_found(dangerous_truncate_sql).should_be_true();
            }

            [Observation]
            public void if_dangerous_script_has_approval_should_return_true()
            {
                sut.is_dangerous_unreviewed_script_found(dangerous_sql_with_override).should_be_false();
            }
        }
    }
}